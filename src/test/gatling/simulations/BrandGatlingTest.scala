...
/**
 * Performance test for the Brand entity.
 */
class BrandGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Brand entity")
        ...
        .repeat(2) {
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_brand_url")))
            .pause(10)
            .repeat(5) {
                exec(http("Get created brand")
                .get("${new_brand_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created brand")
            .delete("${new_brand_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
